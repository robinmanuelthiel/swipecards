...
namespace SwipeCards
{
	public partial class CardStackView : ContentView
	{
		...
		//public static readonly BindableProperty DraggableAreaMarginProperty = BindableProperty.Create(nameof(DraggableAreaMargin), typeof(Thickness), typeof(CardStackView), new Thickness(0), propertyChanged: (BindableObject bindable, object oldValue, object newValue) => { ((CardStackView)bindable).TouchObserver.Margin = (Thickness)newValue; });
		...
		//public Thickness DraggableAreaMargin
		//{
		//	get { return (Thickness)GetValue(DraggableAreaMarginProperty); }
		//	set { SetValue(DraggableAreaMarginProperty, value); }
		//}
		...
		public void Setup()
		{
			...
			// Add two cards (one for the front, one for the background) to the stack. Use inverse direction to ensure that first card is on top
			for (var i = NumberOfCards - 1; i >= 0; i--)
			{
				var cardView = new CardView(ItemTemplate)
				{
					IsVisible = false,
					Scale = (i == 0) ? 1.0f : _defaultSubcardScale //,IsEnabled = false
				};
		...
		private async Task HandleTouchCompleted()
		{
			...
			if (Math.Abs(_cardDistance) >= CardMoveDistance)
			{
				...
				await topCard.TranslateTo(_cardDistance > 0 ? this.Width * 2 : -this.Width * 2, 0, DefaultAnimationLength, Easing.SinIn);
			...
			else
			{
				...
				await Task.WhenAll(
					...
					topCard.TranslateTo((-topCard.X), -topCard.Y, DefaultAnimationLength, Easing.SpringOut),  // Move card back to the center
					topCard.RotateTo(0, DefaultAnimationLength, Easing.SpringOut),
					backCard.ScaleTo(_defaultSubcardScale, DefaultAnimationLength, Easing.SpringOut) // Scale the back card down
				);
			}
		...
		public async Task Swipe(SwipeDirection direction, uint animationLength = DefaultAnimationLength)
		{
			...
			await Task.WhenAll(
				...
				topCard.RotateTo(direction == SwipeDirection.Right ? 17.18873385f : -17.18873385f, animationLength, Easing.SinIn), // Rotate card (57.2957795f/3=17.18873385f)
				backCard.ScaleTo(1.0f, animationLength)  // Scale back card up
			);
